...
class JiraClient:
    ...
    def _setup_authentication(self):
        ...
        if not api_token:
            logger.warning(f"No JIRA API token found in keyring for {self.email}")
            logger.info("Please run 'git-autometa config' to set up your JIRA credentials")
            raise ValueError("JIRA API token not found. Please configure your credentials.")
    ...
    def get_issue(self, issue_key: str) -> JiraIssue:
        ...
        try:
            ...
            logger.error(f"Unexpected error fetching JIRA issue {issue_key}: {e}")
            raise
