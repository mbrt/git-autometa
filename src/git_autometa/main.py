...
def format_branch_name(pattern: str, issue: JiraIssue, max_length: int = 50) -> str:
    ...
    remaining_length = max_length - jira_id_length - 10  # Reserve space for pattern and separators
...
def cli(ctx, config, verbose):
    ...
    try:
        ...
        if verbose:
            console.print(f"[dim]Loaded configuration from: {ctx.obj['config'].config_path}[/dim]")
    except Exception as e:
...
def create(ctx, jira_issue, branch_only, pr_only, base_branch, no_draft, push):
    ...
    try:
        ...
        if not config.jira_server_url or not config.jira_email:
            console.print("[red]JIRA configuration missing. Run 'git-autometa config' first.[/red]")
            sys.exit(1)
        ...
        # Fetch JIRA issue
        console.print(f"[bold blue]Fetching JIRA issue: {jira_issue}[/bold blue]")
        ...
        if not pr_only:
            ...
            if git_utils.branch_exists(branch_name):
                console.print(f"[yellow]Branch '{branch_name}' already exists locally[/yellow]")
...
def status(ctx):
    ...
    console.print(f"JIRA Server: {config.jira_server_url or '[red]Not configured[/red]'}")
    console.print(f"JIRA Email: {config.jira_email or '[red]Not configured[/red]'}")
